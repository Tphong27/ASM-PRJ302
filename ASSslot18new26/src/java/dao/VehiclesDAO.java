...
public class VehiclesDAO {
    ...
    public static boolean isValidPlateNumber(String plateNumber) {
        ...
        // Tách phần số từ biển số (giả sử phần số nằm sau dấu "-")
        String[] parts = plateNumber.split("-");
        if (parts.length != 2) {
            return false; // Định dạng không hợp lệ nếu không có dấu "-"
        }

        String numberPart = parts[1]; // Lấy phần số sau dấu "-"

        // Kiểm tra phần số có độ dài 4 hoặc 5 và chỉ chứa chữ số
        return (numberPart.length() == 4 || numberPart.length() == 5) && numberPart.matches("\\d+");
    }
